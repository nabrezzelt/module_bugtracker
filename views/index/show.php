        ...
        <div class="row">
            ...
            <div class="col-sm-6 text-right">
                <?php
                    ...
                    {
                        echo "<a class='btn btn-default' href='{$this->getUrl(['module' => 'bugtracker', 'controller' => 'index', 'action' => 'edit', 'bug-id' => $bug->getID()])}'>Edit</a> ";
                        echo "<a class='btn btn-danger' href='{$this->getUrl(['module' => 'bugtracker', 'controller' => 'index', 'action' => 'delete', 'bug-id' => $bug->getID()])}''>Delete</a> ";
                    }
                    if(isset($user))
                        echo "<a class='btn btn-success' href='{$this->getUrl(['module' => 'bugtracker', 'controller' => 'index', 'action' => 'new'])}'>New Issue</a>";
                ?>                                            
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body"><?php echo $bug->getDescription(); ?></div>
                </div>
                <?php
                    ...
                    foreach($bug->getComments() as $comment)
                    {
                        ...
                        if(($comment->isInternOnly() && !$user->isAdmin()))
                            continue;
                    ...
                    if(isset($user))
                    {
                        echo "<hr />
                              <form action='{$this->getUrl(['module' => 'bugtracker', 'controller' => 'index', 'action' => 'comment', 'bug-id' => $bug->getID()])}' method='post'>
                                  ...
                                  " . (($user->isAdmin()) ? "<div class='checkbox'><label><input type='checkbox' name='intern-only' value='1' />Intern only</label></div>" : '') . "
